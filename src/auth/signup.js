...
export const signUpcontroller = () => {
  ...
  formContainer.onsubmit = (ev) => {
    ev.preventDefault();

    model.formDataValues.forEach(({ name }) => {
      console.log(ev.target[name].value);
    });

    console.log(ev.target['month'].value);
    console.log(ev.target['day'].value);
    console.log(ev.target['year'].value);
  };

  const submitBtn = DOMElementGenerator.createButtonElement('Submit');
